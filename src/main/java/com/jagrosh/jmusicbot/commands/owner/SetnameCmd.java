...
public class SetnameCmd extends OwnerCommand
{
    ...
    {
        this.name = "setname";
        this.help = "Défini le nom du bot";
        this.arguments = "<name>";
    ...
    protected void execute(CommandEvent event) 
    {
        try 
        {
            ...
            event.reply(event.getClient().getSuccess()+" Nom changé de `"+oldname+"` à `"+event.getArgs()+"`");
        } 
        catch(RateLimitedException e) 
        {
            event.reply(event.getClient().getError()+" Le nom du bot ne peux être changé que deux fois par jours !");
        }
        catch(Exception e) 
        {
